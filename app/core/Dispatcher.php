...
class Dispatcher extends \Phroute\Phroute\Dispatcher{
    ...
    public function dispatch($httpMethod, $uri)
    {
        ...
        if(Cache::get("router",$uri)) {
            list($resolvedHandler, $vars, $beforeFilter, $afterFilter) = Cache::get("router", $uri);
            $response = call_user_func_array($resolvedHandler, $vars);
            return $this->dispatchFilters($afterFilter, $response);
        }

        $routes = $this->dispatchRoute($httpMethod, trim($uri, '/'));

        foreach ($routes as $route) {
            list($handler, $filters, $vars) = $route;
            ...
            if($response != null ) {
                ...
                Cache::set("router",$uri,[$resolvedHandler,$vars,$beforeFilter,$afterFilter]);
    ...
    private function dispatchVariableRoute($httpMethod, $uri)
    {
        ...
        foreach ($this->variableRouteData as $data)
        {
            ...
            foreach ($routes[$httpMethod] as $key=>$subroute) {

                foreach (array_values($routes[$httpMethod][$key][2]) as $i => $varName) {
                    if (!isset($matches[$i + 1]) || $matches[$i + 1] === '') {
                        unset($routes[$httpMethod][$key][2][$varName]);
                    } else {
                        $routes[$httpMethod][$key][2][$varName] = $matches[$i + 1];
                    }
                }
            }
