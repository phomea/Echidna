...
class RouterService extends Service{
    ...
    public static function getRoute( $name ){
        ...
        if( Events::exists(Events::ROUTER_GETROUTE)){
            return Events::dispatch(Events::ROUTER_GETROUTE,$name)[0];
        }else{
    ...
    static function dispatchRoute( $route , $params = [] ){
        ...
        $r['current_url'] = Request::getCurrentUrl();
        ...
        if($response) {
            if( Events::exists(Events::ROUTER_AFTER_DISPATCH)){
                $response = Events::dispatch(Events::ROUTER_AFTER_DISPATCH,$response,$route,$r);
            }
    ...
    static function dispatch(){
        ...
        $query = Events::dispatch(Events::ROUTER_PARSEQUERY_BEFORE,$query)[0];
        ...
        foreach (self::$routes as $key => $route) {
            ...
            if( is_array($r) ){
                ...
                if($response) {
                    ...
                    $p = Request::getParams();
                    unset($p['query']);
                    $route = $route->prepare(array_merge($r,$p));
                    Response::addVariable([
                        "current_route" =>   $route
                    ]);
                    return;
