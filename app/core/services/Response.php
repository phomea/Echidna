...
class Response extends Service {
    ...
    static function getTemplateToUse($template = null, $response = false ,$template_extend = ""){
        ...
        if($mainResponse && strpos(Request::$headers['Accept'],"json")!==false ){// isset(Request::$headers[Request::AJAX_REQUEST_NAME])){
            self::setTemplateToUse("jsonTemplate");
...
//        return new self::$config[self::$templateToUse]( $template , $response );
    ...
    static function go( $url , $header = null ){
        if(!empty($header)){
