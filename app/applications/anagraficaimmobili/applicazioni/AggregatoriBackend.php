...
class AggregatoriBackend extends \core\abstracts\BackendApplication{
    ...
    public static function actionAdd($params = [])
    {
        ...
        if(isset($params['idCondominio'])) {
            ...
            if ($n) {
                ...
                $breadcrumbs[] = ["label" => "Lista aggregatori", "link" => $routeModificaCondominio->build(["id" => $params['idCondominio'], "tab" => 4])];
            }


            $breadcrumbs[] = [
                "label" => "Aggiungi nuovo aggregatore", "link" => ""
            ];
    ...
    {
        $m = parent::actionMod($params); // TODO: Change the autogenerated stub
        $m[1]["title"] = "Modifica Aggregatore: ".$m[1]["data"]->nomeAggregatore;


        $data = [];
        if( $condominio = Condominio::query(true)->where("id=".$m[1]["data"]->idCondominio)->getOne() ){
            ...
            $breadcrumbs[] = ["label"=>$nomeCondomino, "link"=>$linkCondominio];
            $breadcrumbs[] = ["label"=>"Lista aggregatori", "link"=>$routeModificaCondominio->build(["id"=>$condominio->id,"tab"=>4])];

            $breadcrumbs[] = ["label"=>$m[1]["data"]->nomeAggregatore, "link"=>""];
    ...
    public static function actionList($params = [])
    {
        ...
        $m[1]['title'] = "Elenco degli Aggregatori";
        return $m;
    }
    ...
    static function actionDelete($params = [])
    {
        ...
        if(isset($params['idCondominio'])){
            return Response::redirect(RouterService::getRoute(Condominio::getEntity().".mod")->build([
                ...
                "tab"   =>  "4",
                "4-page"  =>  isset($params['page']) ? $params['page'] : 0
            ]));
        }
